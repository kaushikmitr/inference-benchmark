...
for request_rate in $(echo $REQUEST_RATES | tr ',' ' '); do
  ...
  if [[ "$MIN_INPUT_LENGTH" ]]; then
    PYTHON_OPTS="$PYTHON_OPTS --min-input-length  $MIN_OUTPUT_LENGTH"
  fi
  if [[ "$MIN_OUTPUT_LENGTH" ]]; then
    PYTHON_OPTS="$PYTHON_OPTS --min-output-length  $MIN_OUTPUT_LENGTH"
  fi
  if [[ "$DUMMY_URL" ]]; then
